...
def test_virtual_plc_changes_tip_rack_then_add_liquid_continues(tmp_path):
    ...
    try:
        ...
        assert coordinator._tip_changes, first["diagnostics"]
        _wait_until(
            lambda: all(job.future.done() for job in coordinator._tip_changes.values()),
            60,
            f"换料架没有结束: {virtual_plc.errors}",
        )
        ...
        _wait_until(
            lambda: bool(client.succeeded or client.failed or results),
            60,
